...
class Wrapper {
    ...
    finalize() {
        if (!this.Calls) {
            const calls = this.Name.slice("__opus_".length).split("_");
    ...
    to_jsdoc(spacing=1, snake=false) {
        let str = Configuration.SPACING.repeat(spacing) + `/// @function ${this.Name.slice(2)}(${this.Arguments.filter(e => !e.Hidden).map(e => e.Name).join(", ")})\n`;
        for(let i = 0; i < this.Arguments.length; i++) {
            const arg = this.Arguments[i];
    ...
    to_gml(spacing=1, snake=false) {
        let str = Configuration.SPACING.repeat(spacing) + (!snake ? `function ${this.Name.slice(2)}(` : `function ${this.Name.slice(2)}(`) + this.Arguments.filter(e => !e.Hidden).map(e => {
