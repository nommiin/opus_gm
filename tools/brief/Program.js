...
class Program {
     ...
     * @param {string} script Path to opus_gm .gml script
     */
    static main(root, extension, script) {
        Logger.info("Retrieving wrappers...");
        Logger.info("get" + fs.readdirSync(root));
    ...
    static writeWrappers(wrappers, file) {
        ...
        const res_index = extension.files.findIndex(e => e.filename === "opus_gm.dll");//, resource = extension.files.find(e => e.filename === "imgui_gm.dll");
        if (res_index === -1) throw `Could not parse "${file.Name}", expected entry "opus_gm.dll" in extension files`;

        const resource = extension.files[res_index];
        const functions = resource.functions.filter(e => {
            !wrappers.find(w => w.Name.slice(2) === e?.name)
        });
        wrappers.forEach(e => {
    ...
    static writeScripts(wrappers, file) {
        ...
        wrappers.forEach(e => {
            ...
            content.push(e.to_jsdoc(0) + "\n" + e.to_gml(0));
        });

        const final = "/// @section Binds\n" + content.join("\n") + "\n" + file.Content.slice(start);
